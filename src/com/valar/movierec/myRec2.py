...
class CF:
    ...
    def showTable(self):
        ...
        rows = []
        rows.append([u"MovieID", u"YearOfRelease", u"Title", u"from CustomerID"])
...
def readRatingsFile(rootdir):
    data = [] #data should be like : CustomerID,movieID,Rating,Date
    ...
    for i in range(0,len(list)):
       ...
       if os.path.isfile(path):
           ...
           tempMovieID = -1
           for line in files.readlines():
               if tempPosi == True:#first line should be movie id
                   item = line.strip().split(":")
                   tempMovieID = item[0]
               else: # other lines should be like : CustomerID,Rating,Date
                   item = line.strip().split(",")
                   item.insert(1,tempMovieID)
                   data.append(item)
               tempPosi = False
    return data      
...
# -------------------------开始-------------------------------
start = time.clock()
movies = readMoviesFile("D:/software/Python/Movie data/hunter data/download/download/movie_titles.txt")
ratings = readRatingsFile("D:/software/Python/Movie data/hunter data/download/download/training_set_test")
demo = CF(movies, ratings, k=20)
print('recommend for customer:')
customerRecID = "124105"
print(customerRecID)
demo.recommendByUser(customerRecID)
print("推荐列表为：")
demo.showTable()
